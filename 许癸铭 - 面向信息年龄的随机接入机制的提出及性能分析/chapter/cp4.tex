...
\subsection{仿真验证}
在这一部分，我们通过 MATLAB 编写实验仿真代码，对前文所推导的内容进行验证。实验主要分为两部分：首先做的是验证平均 AoI 与 AoI 违规概率理论闭式解的准确性；随后，将本文提出的等待 CSMA 机制与现有的相关 CSMA 算法进行横向对比，以评估所提机制在提升系统信息时效性方面的优越性。

\subsubsection{理论推导闭式的准确性验证}
为了验证理论模型的正确性，我们设定网络中的节点数量 $N$ 在 100 到 200 之间动态变化（步长为 10）。系统的参数配置如下：单个忙碌时隙与空闲微时隙的比例系数设定为 $M = 150$，主动退避窗口 $z = 64$，竞争窗口动态适配为 $w = 2N - 1$，同时，用于评估极端老化风险的 AoI 违规阈值设定为 $\omega = 1.5N$。
...
\begin{figure}[H]
    ...
    \begin{minipage}[t]{0.42\textwidth} % 【修改1】把 0.48 改小，例如 0.42
        \centering
        \includegraphics[width=\textwidth]{figure/cp4/平均AoI验证.pdf}
    ...
    \begin{minipage}[t]{0.42\textwidth} % 【修改1】把 0.48 改小，例如 0.42
        \centering
        \includegraphics[width=\textwidth]{figure/cp4/AoI违规概率验证.pdf}
    ...
    \caption{理论与仿真对比}
    \label{fig:cp4_prove} % 【修改3】\label 必须放在 \caption 之后！（建议加上 fig: 前缀方便管理）
\end{figure}

图\ref{fig:cp4_prove}的（a）与（b）分别展示了等待 CSMA 在上述参数配置下，平均 AoI 与 AoI 违规概率的理论计算曲线与实际仿真值的对比结果。通过观察可以清晰地发现，无论是衡量系统常态新鲜度的平均 AoI，还是刻画系统极端老化风险的违规概率，理论闭式解的曲线与代表实际系统运行的仿真散点均实现了高度重合。

\subsubsection{算法性能横向对比评估}
在验证了理论闭式之后，我们将等待 CSMA 算法与标准的 802.11 DCF 机制以及睡眠 CSMA 和 Fresh CSMA 机制进行对比。为了更好地观察算法在中小规模到大规模网络演进中的表现，我们设定设备数量 $N$ 的变化范围为 10 到 100（步长为 10），且 AoI 违规阈值设定为 $\omega = 1.5N$。这三种机制的具体运行规则如下：
\begin{enumerate}[wide]
    \item 802.11 DCF：这是现有的802.11 DCF 机制，它仅具有竞争窗口。在节点每次成功传输数据包后，竞争窗口被设置为最小值 $CW_{min}$；而在每次传输失败后，竞争窗口的大小将翻倍，直至达到最大值 $2^mCW_{min}$。我们在仿真中将最小竞争窗口设置为 $CW_{min}=64$，并将最大退避层级设置为 $m=4$。
    \item 睡眠 CSMA：在节点每次成功传输后，该节点进入睡眠模式，退出信道竞争，睡眠时长为一个服从指数分布的随机数，否则，每个节点使用固定的竞争窗口进行信道的竞争。我们将竞争窗口设置为 $w=2N-1$，并通过穷举搜索找到最佳的平均睡眠时长。
    \item Fresh CSMA：在节点每次传输数据包后，无论成功与否，所有的节点都会根据当前的 AoI 随机选择新的退避计时器。我们参照文献\cite{tripathi2023fresh}，设置跟其一样的仿真参数。
\end{enumerate}
\begin{figure}[H]
    \centering
    % === 第一张图 (左) ===
    \begin{minipage}[t]{0.42\textwidth} % 【修改1】把 0.48 改小，例如 0.42
        \centering
        \includegraphics[width=\textwidth]{figure/cp4/平均AoI对比_高清.pdf}
        
        \vspace{6pt}
        {\zihao{5}\songti (a) 不同机制的平均AoI对比}
    \end{minipage}%
    \hspace{0.06\textwidth} % 【修改2】用固定宽度替换 \hfill，控制两图中间的间隙
    % === 第二张图 (右) ===
    \begin{minipage}[t]{0.42\textwidth} % 【修改1】把 0.48 改小，例如 0.42
        \centering
        \includegraphics[width=\textwidth]{figure/cp4/AoI违规概率_高清.pdf}
        
        \vspace{6pt}
        {\zihao{5}\songti (b) 不同机制的AoI违规概率对比}
    \end{minipage}
    
    \caption{不同机制的性能对比}
    \label{fig:cp4_compare} % 【修改3】\label 必须放在 \caption 之后！（建议加上 fig: 前缀方便管理）
\end{figure}

首先是平均AoI的对比，如图\ref{fig:cp4_compare}的 (a) 图所示，等待 CSMA 的表现最佳，睡眠 CSMA 次之，这是因为睡眠 CSMA 在节点成功传输数据包后，有可能随机到一个极短的睡眠时长，这种情况下并不能为信道的减负带来增益，相反，等待 CSMA 强制成功传输的节点至少退避 $z$ 个时隙，能够保证其他的节点拥有更多的机会成功接入信道。Fresh CSMA 在节点数量不大于30时表现与睡眠 CSMA 相当，却在节点数量超过30后呈现出较差的特性，这是因为Fresh CSMA 没有考虑碰撞的影响，Fresh CSMA 会根据节点的 AoI 选择退避计时器，当 AoI 高时，Fresh CSMA 会选择一个较短的退避计时器，而当 AoI 较低时，则选择一个较长的退避计时器，当节点数量过多时，由于无休止的碰撞，绝大部分甚至全部节点都处于较高的 AoI，根据 Fresh CSMA 的设计，它将会为它们选择极短的退避计时器，甚至是0，而这非但无法缓解碰撞的现象，反而会进一步加剧碰撞，从而导致图中的现象。

接下来是 AoI 违规概率的对比，如图\ref{fig:cp4_compare}的 (b) 图所示，从图中可以观察到，等待 CSMA 在抑制极端信息陈旧方面表现出最佳的性能，睡眠 CSMA 表现次之，Fresh CSMA 在节点数量超过30后，性能急转直下，原因与前面平均 AoI 时所分析的一样。

\subsection{小结}
本章将信息时效性的研究从理想的等长时隙拓展到了更贴近实际底层物理机制的变长时隙 CSMA 环境中。针对 802.11 DCF 机制盲目竞争的弊端，将等待机制应用到了 802.11 DCF 上，设计出了等待 CSMA 协议。该协议通过强制成功传输的节点主动退避一段时间，为全网让渡传输机会，同时保留了碰撞后的随机退避窗口来降低碰撞概率。

在性能评估方面，本章利用解耦假设构建了一维马尔可夫链模型，推导出了系统的稳态自洽方程，并论证了稳态工作点的存在性与唯一性。随后，基于 AoI 演化的微观几何特征与矩匹配近似理论，成功推导出了该机制下平均 AoI 与 AoI 违规概率的闭式解析表达式。最终的仿真实验不仅证实了上述所有数学推导的极高准确性，更通过与其他 CSMA 算法的横向对比，充分证明了等待 CSMA 机制在抑制信息老化与降低极端时效性违规风险上的全面优越性。