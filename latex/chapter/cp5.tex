...
为便于比较，本章将实验分为两类。第一类用于验证理论表达式的准确性，分别考察节点数量 $N$ 和等待窗口 $z$ 变化时理论值与仿真值的一致程度；第二类用于比较不同机制在同一负载条件下的性能表现。ALOHA 类机制的对比对象包括经典时隙 ALOHA 和文献\upcite[1456]{yavascan2021analysis}提出的阈值 ALOHA；CSMA 类机制的对比对象包括标准 CSMA、文献\upcite[818]{maatouk2020age}提出的睡眠 CSMA，以及文献\upcite[556]{tripathi2023fresh}提出的 Fresh CSMA。节点规模的验证图主要用来观察网络负载提高后理论模型是否仍能贴合仿真结果，等待窗口的验证图则关注等待窗口的变化对 AoI 性能的影响。

本章的仿真设置旨在分别从网络负载与等待参数两个维度考察系统性能，从而更清楚地验证理论模型的有效性，并为后续的机制对比提供统一依据。

\subsection{理论表达式验证}

\subsubsection{等待ALOHA}
...
综合图\ref{fig:cp5_aloha_comparison}与图\ref{fig:cp5_csma_comparison}可以看出，等待随机接入退避机制并不依赖某一种特定的底层 MAC 框架。无论是在等长时隙的 ALOHA 环境中，还是在具备载波侦听和变长系统时隙的 CSMA 环境中，适度限制成功节点的再次接入，并对碰撞后的节点进行处理，都是改善 AoI 性能的有效思路。
