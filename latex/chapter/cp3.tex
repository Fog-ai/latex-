...
\begin{enumerate}[wide]
    \item 初始化：取左右端点 $l_0=0$、$r_0=1$，并给定收敛阈值 $\epsilon>0$；
    \item 区间折半：在第 $t$ 轮迭代中计算中点 $m_t=(l_t+r_t)/2$，并求取 $F(m_t)$；
    \item 区间更新：若 $F(m_t)=0$，则 $m_t$ 即为所求；若 $F(m_t)<0$，则令 $l_{t+1}=m_t,\ r_{t+1}=r_t$；否则令 $l_{t+1}=l_t,\ r_{t+1}=m_t$；
    \item 停止准则：当 $|r_t-l_t|<\epsilon$ 时停止迭代，并以区间中点作为 $p_{cl}^\star$ 的数值近似。
\end{enumerate}
