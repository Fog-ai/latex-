...
本章考虑一个由单个接入点（Access Point, AP）和 $N$ 个终端组成的无线网络，该网络采用时隙化 ALOHA 的接入方式，所有终端在公共信道上竞争传输机会，其系统模型如图\ref{fig:cp3_system_model}所示。

\begin{figure}[htbp]
    \centering
    \input{figure/cp3/system_model_aloha_tikz.tex}
    \caption{系统模型图}
    \label{fig:cp3_system_model}
\end{figure}
...
基于这一考虑，本文提出一种基于等待的随机接入退避机制，并将其引入时隙 ALOHA 中，构造等待 ALOHA 协议。其核心规则包括：

1. \textbf{成功传输后的确定性等待。} 当终端成功完成一次状态更新后，协议强制其在接下来的 $z$ 个时隙内不参与信道竞争。参数 $z$ 为等待窗口长度，其作用是抑制刚完成更新的终端再次接入信道，让尚未完成更新的终端获得更多的传输机会。

2. \textbf{碰撞后的随机退避。} 当终端发送失败时，仍沿用传统 ALOHA 中的随机退避机制，即生成服从 $\beta \sim \text{unif}(0,w-1)$ 的退避计时器，以降低后续时隙中重复碰撞的概率。
...
1. 终端保持静默，不发起传输。状态从 $(0,k)$ 转移到 $(0,k+1)$，转移概率为 $1-p_0$；

2. 终端发起传输但发生碰撞。此时状态从 $(0,k)$ 转移到 $(i,k+1)$，其中 $i \in [0,w-1]$，转移概率为 $q=\frac{p_0 p_{cl}}{w}$；

3. 终端成功发送更新包。此时 AoI 被重置为 1，退避计时器被置为 $z$，即状态从 $(0,k)$ 转移到 $(z,1)$，转移概率为 $p_s=p_0(1-p_{cl})$。

由于成功发送后的终端必须等待 $z$ 个时隙，因此系统存在一条确定性的状态链 $(z,1)\to\cdots\to(0,z+1)$。这意味着在稳态下，终端重新进入可竞争状态时，其 AoI 必然不小于 $z+1$。基于上述转移规则，可以得到完整的二维马尔可夫链，如图\ref{fig:cp3_aloha_system_tran_photo}所示。为简化书写，下文记 $p=(1-p_0)+\frac{p_0p_{cl}}{w}$，并记 $\omega$ 为预设的 AoI 门限。
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{figure/cp3/系统的状态转移图.pdf}
    \caption{系统的状态转移图}
...
\end{equation}
另一方面，由于 $\tau(1)=\frac{p_0}{1+zp_0+p_0\left(\frac{w-1}{2}-z\right)}=\frac{p_0}{1+\frac{p_0(w-1)}{2}}\in(0,1)$，因此
\begin{equation}
...
进一步考察唯一性所需的单调性条件。记 $D(x)=1+zp_0+p_0x\left(\frac{w-1}{2}-z\right)$，则
\begin{equation}
...
当 $w\ge 2z+1$，即 $\frac{w-1}{2}\ge z$ 时，有 $\tau'(x)\le 0$，于是 $f'(x)\le 0$，$x\in[0,1]$。也就是说，$f(x)$ 在 $[0,1]$ 上单调不增。由此立即得到 $F'(x)=1-f'(x)\ge 1>0$，即 $F(x)$ 在 $[0,1]$ 上严格单调递增。

结合前述结论 $F(0)<0$ 与 $F(1)>0$，可知 $F(x)$ 只能与横轴相交一次，因此方程\eqref{cp3_pcl}在区间 $[0,1]$ 上存在唯一解。换言之，当随机退避窗口满足 $w\ge 2z+1$ 时，等待 ALOHA 的稳态碰撞概率被唯一确定。

相对地，当 $w<2z+1$，即 $\frac{w-1}{2}<z$ 时，有 $\tau'(x)>0$，则 $f'(x)>0$。此时 $f(x)$ 随 $x$ 增大而增大，$F'(x)=1-f'(x)$ 未必始终为正。虽然方程仍然因连续性而至少存在一个解，但仅凭上述分析已无法保证解的唯一性。特别是在网络规模 $N$ 较大时，因子 $(N-1)$ 会放大 $f'(x)$ 的数值，使得 $f'(x)$ 超过 1 的可能性显著增加，进而导致 $F(x)$ 失去单调性。因此，$w<2z+1$ 时不能直接推出唯一解的结论。

综上所述，在满足 $w\ge 2z+1$ 的条件下，函数 $F(x)$ 在 $[0,1]$ 上连续且严格递增，并满足 $F(0)<0<F(1)$。因此，二分法可以稳定求得唯一根 $p_{cl}^\star$，其步骤如下：

1. 初始化：取左右端点 $l_0=0$、$r_0=1$，并给定收敛阈值 $\epsilon>0$；

2. 区间折半：在第 $t$ 轮迭代中计算中点 $m_t=(l_t+r_t)/2$，并求取 $F(m_t)$；

3. 区间更新：若 $F(m_t)=0$，则 $m_t$ 即为所求；若 $F(m_t)<0$，则令 $l_{t+1}=m_t,\ r_{t+1}=r_t$；否则令 $l_{t+1}=l_t,\ r_{t+1}=m_t$；

4. 停止准则：当 $|r_t-l_t|<\epsilon$ 时停止迭代，并以区间中点作为 $p_{cl}^\star$ 的数值近似。
...
1. 求解 $Y$ 的一阶矩
    ...
    在明确 $Y_k$ 的统计规律后，为了求解 $Y$ 的整体数学期望，将式\eqref{cp3_alpha_expression}与式\eqref{cp3_EYk}进行联立，根据全期望公式展开并化简，可得
...
2. 求解 $Y$ 的二阶矩
    ...
    \begin{equation}\label{cp3_EYK^2_withAB}
        ...
        \end{aligned}
    \end{equation}
    其中，$U=z-\frac{w-1}{2}$，$V=\frac{1}{p_0}+\frac{w-1}{2}$。
...
考虑到 $Y$ 必然取正值，且在碰撞与退避作用下通常呈现右偏和长尾的特征，因此采用移位指数分布作为近似模型更为自然。设 $Y\sim c+\mathrm{Exp}(\lambda)$，则其概率密度函数为
...
根据移位指数分布的统计特性，有 $E[Y] = c + \frac{1}{\lambda}$，以及 $D[Y] = E[Y^2] - (E[Y])^2 = \frac{1}{\lambda^2}$。
...
由图\ref{fig:cp3_aoi_evolution_with limit}可知，峰值 AoI 为 $1+Y$，故违规量为 $J=\max(0,1+Y-\omega)$。
记 $H=\omega-1$，则
...
\end{aligned}
\end{equation}
化简可得 $E[J]  = \frac{1}{\lambda} e^{-\lambda(H-c)}$。与式\eqref{cp3_lambdaandc}联立，得到
\begin{equation}\label{cp3_EJresult_expand}
\begin{aligned}
E[J] &= \frac{1}{\lambda} \exp\Big\{ -\lambda \big[ \omega - 1 - \big( E[Y] - \frac{1}{\lambda} \big) \big] \Big\} \\
&= \frac{1}{\lambda} \exp\big[ -\lambda(\omega - 1 - E[Y]) - 1 \big]
\end{aligned}
\end{equation}
由于 $\lambda=\frac{1}{\sqrt{D[Y]}}$，将上式化简为
\begin{equation}\label{cp3_EJresult}
E[J] = \sqrt{D[Y]} \exp\Bigg( -\frac{\omega - 1 - E[Y]}{\sqrt{D[Y]}} - 1 \Bigg)
\end{equation}
...
在得到平均 AoI 与 AoI 违规概率的闭式表达式后，本节将进一步讨论等待 ALOHA 的性能表现。为简化书写，记 $\mu_Y = E[Y] = z + M$，$\sigma_Y^2 = D[Y] = C_0 - M^2$。
...
1. $1 + \frac{\mu_Y}{2}$ 反映了平均离开时间间隔对 AoI 的基本影响；

2. $\frac{\sigma_Y^2}{2\mu_Y}$ 反映了离开时间间隔波动带来的额外惩罚。

这一定量分解表明，在 AoI 优化问题中，仅仅缩短平均离开时间间隔并不足够；若离开时间间隔伴随着较大的波动，平均 AoI 仍会显著恶化。等待 ALOHA 中引入的确定性等待窗口 $z$，本质上正是通过对刚完成更新的终端施加可控约束，以此来换取全网竞争强度的下降和离开时间间隔方差的收缩。对面向 AoI 的随机接入退避机制的设计而言，这种“以确定性换稳定性”的思路是其区别于传统吞吐量导向设计的关键所在。
...
1. 系数 $\frac{\sigma_Y}{\mu_Y}$ 是离开时间间隔的变异系数，这个系数说明违规概率不仅取决于离开时间间隔的期望值，而且也受离开时间间隔离散程度的直接影响；

2. 当门限 $\omega$ 大于 $1+\mu_Y$ 时，违规概率关于门限差值呈指数衰减，而衰减快慢由 $\sigma_Y$ 决定。$\sigma_Y$ 越大，尾部衰减越慢，极端陈旧状态出现的概率则越高。

因此，等待 ALOHA 对违规概率的改进并不只体现为平均意义上的 AoI 降低，更体现在对极端陈旧状态这一尾部风险的抑制。特别是在高负载场景下，无线信道的资源十分受限，若不对盲目且激进的竞争行为进行额外约束，由频繁碰撞所引发的连续退避会迅速拉长右尾，最终导致 AoI 违规概率呈现出显著上升的恶化趋势。等待 ALOHA 通过调节节点在完成更新与碰撞后的再次接入行为，有效抑制了连续碰撞的产生，从而缓解了拥塞环境下极端陈旧状态的发生。
...
因此，$z$ 的作用具有典型的权衡特征：一方面，它引入了额外的等待开销；另一方面，它减少了随机碰撞带来的不确定性时延。对于终端规模较大的接入场景，后者往往更为重要，因为系统性能的主要恶化来源并不是固定等待本身，而是碰撞所诱发的长尾退避过程。适当增大 $z$，实质上是将不可控的随机延迟转换为可控的确定性延迟，从而在整体上改善平均 AoI，并降低 AoI 违规风险。

\subsection{本章小结}

本章围绕等待 ALOHA 的设计与分析展开。针对经典时隙 ALOHA 无法区分不同终端时效需求的问题，本文在成功传输后引入确定性等待机制，建立了等待 ALOHA 的系统模型，并在按需生成的采样假设下刻画了其 AoI 演化与退避行为。进一步地，借助解耦近似与马尔可夫链建模，本文给出了碰撞概率的稳态方程，并证明了在 $w\ge 2z+1$ 条件下该方程解的存在性与唯一性。

在此基础上，本章以离开时间间隔 $Y$ 为分析主线，推导了平均 AoI 与 AoI 违规概率的闭式表达，并从方差惩罚、尾部衰减以及等待窗口权衡等角度解释了等待机制改善信息时效性的原因。上述结果表明，适度等待并不会损害信息的新鲜度，反而是改善信息新鲜度的有效途径。
