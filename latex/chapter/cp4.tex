...
为了进一步求解系统稳态下的碰撞概率，将式\eqref{cp4_p_and_s0}代入式\eqref{cp4_s_0_and_p}，消去中间变量 $s_0$，可得到关于 $p$ 的稳态方程
...
1. 求解 $Y$ 的一阶矩
    ...
    设在一个完整的离开时间间隔 $Y$ 内，目标终端共发起了 $\alpha$ 次传输尝试。基于稳态解耦的理论前提，终端每次尝试所经历的碰撞概率均被近似为常数 $p$，因此总传输次数 $\alpha$ 在统计上服从参数为 $1-p$ 的几何分布，其对应的概率质量函数为
...
2. 求解 $Y$ 的二阶矩
    ...
    为了简化后续复杂的代数表达式书写，记两个中间变量：$\phi=z\left(p+\frac{1-p}{M}\right)$，$\gamma=\frac{w-1}{2}\left(p+\frac{1-p}{M}\right)$。
...
&\quad + k(k - 1)(1 + \gamma)^2 + kE\left[V^2\right].
    ...
    最后，为了求解 $Y$ 的二阶矩，将式\eqref{cp4_EYk2}与式\eqref{cp4_alpha}联立，根据全期望公式展开并化简，可得
