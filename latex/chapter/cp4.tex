...
\section{等待 CSMA 的提出及分析}

第三章已经在时隙 ALOHA 中引入了等待随机接入机制，并给出了相应的理论分析。若进一步将这一思路推广到 IEEE 802.11 DCF 网络中的话，问题就不仅只是“是否发送”，而且还要处理载波侦听、退避冻结与恢复等机制带来的影响。换言之，等待机制仍然用于改善信息时效性，但其协议形式和分析方法都需要结合 DCF 的竞争规则重新建立。
...
本章考虑一个由单个接入点（Access Point, AP）和 $N$ 个终端组成的无线网络。与第三章采用时隙 ALOHA 不同，这里的终端按照 IEEE 802.11 DCF 的协议接入信道，即终端在发送前先进行载波侦听，并通过退避计时器协调竞争过程，系统结构如图\ref{fig:cp4_system_model}所示。
\begin{figure}[h]
\centering
...
IEEE 802.11 DCF 采用基于载波侦听的分布式接入方式。终端在准备发送数据前，首先会持续地侦听无线信道，只有当信道在一个 DCF 帧间隔（Distributed Inter-frame Spacing, DIFS）内保持空闲时，终端才被允许进入退避阶段。进入退避阶段后，终端从竞争窗口中随机选取一个整数退避值，并以微时隙为单位开始倒数。若当前微时隙内信道保持空闲，则退避计时器减 1；若有其他终端开始发送，则信道转为忙碌，当前终端立即冻结其退避计时器，待信道又一次在一个 DIFS 内保持空闲后，退避计时器才恢复计数。计时器减至零的终端获得发送机会，若多个终端在同一时刻归零，则会在同一忙碌时段内发送并产生碰撞。由此可见，DCF 的核心不只是随机退避本身，还包括“侦听、冻结、恢复与发送”这一循环竞争过程，如图\ref{fig:cp4_csma_and_system_slot}所示。
\begin{figure}[h]
\centering
...
与第三章相同，本文仍然采用按需生成（generate-at-will）的采样机制。终端仅在决定发起传输时才对物理过程进行采样并生成新的状态更新包，因此发送端不维护缓存队列，其排队时延可以忽略不计。根据 AoI 的定义，终端 $i$ 在第 $t$ 个系统时隙的 AoI 演化为
\begin{equation}
    \Delta_i(t + 1) =
\begin{cases}
...
在 mMTC 场景中，CSMA 网络同样面临着与第三章类似的问题：刚完成更新的终端在短时间内继续占用信道，对降低 AoI 的帮助十分有限；而那些长期未成功传输的终端，其在接收端处的状态信息往往已经过时。基于这一点，本文将等待随机接入机制引入 802.11 DCF，构造等待 CSMA 协议。其规则如下：

\textbf{（1）成功传输后的混合退避。} 当终端成功完成一次状态更新后，不再像第三章那样只经历固定的 $z$ 个时隙等待后立即重新接入信道，而是先主动退避 $z$ 个时隙，再从区间 $[0,w-1]$ 中随机选择一个退避计时器 $\beta$。也就是说，新的退避计时器落在区间 $[z,z+w-1]$ 上。其中，参数 $z$ 用于抑制刚完成更新的终端在短时间内重新参与竞争，而随机退避部分则保留了 DCF 原有的接入机制，用于降低碰撞概率。

\textbf{（2）碰撞后的随机退避。} 当终端发送失败时，仍按照 DCF 的方式随机生成 $\beta \sim \text{unif}(0,w-1)$ 的退避计时器，以降低后续重复碰撞的概率。
...
\end{cases}
\end{equation}
其中，$b_j(t)$ 表示除终端 $i$ 之外的其他终端在 $t$ 时刻下的退避计时器状态。
...
\begin{enumerate}[wide]
    \item 当目标终端处于状态 $s_0$ 且发送成功时，它将在区间 $[z,z+w-1]$ 内重新选择退避计时器。由于成功概率为 $1-p$，故对应状态的转移概率为 $\frac{1-p}{w}$。
    \item 当目标终端处于状态 $s_0$ 且发生碰撞时，它将在区间 $[0,w-1]$ 内重新选择退避计时器。由于碰撞概率为 $p$，故对应状态的转移概率为 $\frac{p}{w}$。
    \item 当目标终端处于非零状态时，退避计时器按系统时隙逐步减 1，因此系统将以概率 1 从 $s_m$ 转移到 $s_{m-1}$。
\end{enumerate}
...
\end{equation}
与第三章相比，这一稳态方程的表达形式有所变化。这是因为在等待 CSMA 中，终端成功更新后并不是直接回到固定的等待状态，而是先主动退避 $z$ 个时隙后，再进入随机退避竞争。与此同时，系统的时隙长度还会随信道状态而变化，因此，这里不能直接沿用第三章中的写法及论证方式，在下一小节将重新讨论解的存在性与唯一性。

\subsubsection{稳态方程解的存在唯一性论证}

尽管式\eqref{cp4_ziqiafangcheng_p}的形式不同于第三章中的稳态方程，但仍可通过变量代换的方式来说明其在定义域 $(0,1)$ 上解的存在性与唯一性。为此，引入中间变量
\[
x=\sqrt[N-1]{1-p},
\]
由于 $0<p<1$，故有 $0<x<1$。此时式\eqref{cp4_ziqiafangcheng_p}可改写为
...
\end{equation}
记式\eqref{cp4_ziqiafangcheng_z}右端为 $H(x)$。由于 $z>0$，则必须有 $2-(w+1)(1-x)>0$，换言之，$x$ 必须满足
\[
\frac{w-1}{w+1}<x<1.
\]
...
在确定稳态碰撞概率 $p$ 后，便可进一步分析等待 CSMA 下的平均 AoI 与 AoI 违规概率。与第三章一样，这里的求解路径仍然是先求离开时间间隔 $Y$ 的一阶矩和二阶矩，再代入第二章的统一公式。不同之处在于，等待 CSMA 中成功更新后的终端并不是简单返回到固定等待状态，而是先主动退避 $z$ 个时隙，再进入 DCF 的随机退避竞争。同时，系统的时隙长度也会随信道状态而变化，因此 $Y$ 的构成方式与第三章并不相同。

\subsubsection{平均 AoI 的闭式推导}

与第三章一致，在按需生成的采样机制下，每次成功更新后 AoI 都被重置为 1，因此平均 AoI 的计算公式仍可重写为
\begin{equation}\label{cp4_avg_aoi}
    \Delta = 1 + \frac{E[Y^2]}{2E[Y]}.
\end{equation}
其中，$Y$ 表示相邻两次成功更新之间的离开时间间隔。因此，平均 AoI 的推导可归结为求解 $Y$ 的一阶矩和二阶矩。
...
\begin{enumerate}[wide]
    ...
    与第三章相比，这里的离开时间间隔由等待阶段和竞争阶段共同构成，写法上需要稍作调整。为便于求矩，本文将 $Y$ 分解为两部分：$U$ 表示终端成功更新后主动退避 $z$ 个系统时隙所对应的等待时间，$V$ 表示终端完成一次发送尝试所经历的竞争时间。
    ...
    若在相邻的两次成功更新之间，前 $k-1$ 次尝试均碰撞，第 $k$ 次尝试成功，将对应的离开时间间隔记为 $Y_k$，则根据等待 CSMA 的工作机制，有
    \begin{equation}\label{cp4_Yk_expression}
        Y_k = U+\sum_{j=1}^{k}V_j.
    \end{equation}
    因此，
    ...
    为了简化后续书写，记
    \[
    \phi=z\left(p+\frac{1-p}{M}\right),\qquad
    \gamma=\frac{w-1}{2}\left(p+\frac{1-p}{M}\right).
    \]
...
与第三章类似，AoI 违规概率的求解仍从违规量
\[
J=\max(0,1+Y-\omega)
\]
出发。由于接收端观察到的 AoI 曲线仍保持锯齿状结构，矩匹配与移位指数分布近似的方法仍然适用。与第三章不同的地方只在于 $Y$ 的矩表达式不同，而求解步骤本身保持一致。
...
由移位指数分布的统计特性，有
\begin{equation}
    E[Y] = c + \frac{1}{\lambda},\qquad D[Y] = \frac{1}{\lambda^2}.
\end{equation}
结合式\eqref{cp4_EY}和式\eqref{cp4_EY2result}，并注意到
\[
D[Y] = E[Y^2] - (E[Y])^2 = C_1,
\]
可得
...
\begin{equation}
    \begin{aligned}
        E[J] &= E[\max(0,Y-H)] \\
             &= \int_H^\infty (y-H)\lambda e^{-\lambda(y-c)}dy \\
             &= \frac{1}{\lambda}e^{-\lambda(H-c)}.
    \end{aligned}
\end{equation}
...
在得到平均 AoI 与 AoI 违规概率的闭式表达式后，本节进一步讨论等待 CSMA 的性能表现。为简化书写，记
\[
\mu_Y = E[Y] = \phi + \frac{1+\gamma}{1-p},\qquad
\sigma_Y^2 = D[Y] = C_1.
\]
...
\begin{enumerate}[wide]
    \item $1 + \frac{\mu_Y}{2}$ 反映平均离开时间间隔对 AoI 的基本影响；
    \item $\frac{\sigma_Y^2}{2\mu_Y}$ 反映离开时间间隔波动带来的额外惩罚。
\end{enumerate}

与第三章相比，这一分解在形式上完全一致，但物理含义更为复杂。第三章中的等待窗口 $z$ 对应的是固定长度的离散时隙，而本章中的 $z$ 对应的是固定数量的系统时隙，这些系统时隙在物理时间上可能是长度为 $\frac{1}{M}$ 的空闲微时隙，也可能是长度为 1 的忙碌时隙。因此，即便 $z$ 的取值相同，其转化为实际等待时长后也会随网络负载而发生变化。由此可见，在等待 CSMA 中，不仅碰撞会拉大 $\sigma_Y^2$，系统时隙的变长特性本身也会加剧离开时间间隔的波动。
...
\begin{enumerate}[wide]
    \item 前置系数 $\frac{\sigma_Y}{\mu_Y}$ 是离开时间间隔的变异系数，这表明违规概率不仅由平均离开时间间隔决定，也直接受其离散程度影响；
    \item 当门限 $\omega$ 大于 $1+\mu_Y$ 时，违规概率关于门限差值呈指数衰减，而衰减快慢由 $\sigma_Y$ 决定。
\end{enumerate}
...
不过，$z$ 在本章中的作用与第三章并不完全相同。等待 ALOHA 中，成功终端经过 $z$ 个等长时隙后即可重新接入信道；而在等待 CSMA 中，成功终端经过 $z$ 个系统时隙后，还需要继续经历 DCF 原有的随机退避 $\beta$。因此，$z$ 在这里起到的是“推迟重新进入竞争阶段”的作用，而不是完全替代后续竞争过程。此外，由于系统的时隙长度会随网络状态而变化，因此同样的 $z$ 在负载不同的情况下所对应的实际等待时间并不相同。
...
本章将等待随机接入机制进一步引入 802.11 DCF 环境中，构造了等待 CSMA 协议。与第三章相比，本章模型包含两个关键变化：一是终端成功传输后并非只进行固定的 $z$ 个时隙等待，而是先主动退避 $z$ 个时隙，再进入 DCF 的随机退避竞争；二是系统时隙长度随网络状态变化，离开时间间隔的物理时间不再由固定时隙简单累加得到。

在此基础上，本文建立了目标终端的一维马尔可夫链模型，推导了稳态碰撞概率所满足的稳态方程，并给出了其解存在且唯一的条件。进一步地，本章以离开时间间隔 $Y$ 为分析主线，推导了平均 AoI 与 AoI 违规概率的闭式表达，并讨论了等待窗口、竞争强度以及系统时隙变长特性对性能的共同影响。结果表明，将等待机制引入 CSMA 后，虽然推导路径与第三章保持一致，但参数的物理含义和性能权衡都发生了变化，这也说明本文所提的等待随机接入机制可以在不同 MAC 框架下形成具有针对性的实现形式。
